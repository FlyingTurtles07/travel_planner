...
### .1 기존 오류처리에 더 어떤 단계에 문제가 발생 했는지 " errors"에 일관되게 기록하도록 개선
기존 API 키 없음, LLM 오류, JSON 파싱 실패, Kakao 오류, 검색 결과 0건, 네트워크 오류를 처리에 더 개선하여...
어떤 단계에 문제가 발생 했는지 " errors"에 일관되게 기록하도록
